...
class Validator:
    ...
    def parse_note(self, notes: Information) -> List[str]:
        ...
        elif self.check_work_experience(notes.work_experience) == 0:
            incorrect_keys.append('work_experience')
    ...
    def check_work_experience(self, work_experience:str) -> bool:
        """"Функция, которая провереряет Стаж работы людей на валидность"""

        try:
            work_experience1 = int(work_experience)
        except ValueError:
            return False
        return 0 <= work_experience1 < 50
...
if len(sys.argv) != 1:
    ...
    parser.add_argument('-output_file', nargs=1, type=str, default="correct_data.txt")
